...
namespace UnityEditor.Localization.UI
{
    ...
    class LocalizationTableEditor : UnityEditor.Editor
    {
        ...
        class Styles
        {
            public static readonly GUIContent addToCollection = EditorContent.TextContent("Add Table to Collection", "Adds the table to the collection that shares the same Shared Table Data");
            public static readonly GUIContent createAssetTableCollection = EditorContent.TextContent("Create Asset Table Collection", "Creates a new asset table collection from the loose tables.");
            public static readonly GUIContent createStringTableCollection = EditorContent.TextContent("Create String Table Collection", "Creates a new string table collection from the loose tables.");
            public static readonly GUIContent missingEntriesAdd = EditorContent.TextContent("Add to Shared Data", "Add the entries to the Shared Table Data so that they can be used.");
            public static readonly GUIContent missingEntriesRemove = EditorContent.TextContent("Remove from Table", "Removed the entries from the table");
            public static readonly GUIContent removeFromCollection = EditorContent.TextContent("Remove Table from Collection", "Removes the table from the collection so that is is not used in the project");
            public static readonly GUIContent removeTableFromList = new GUIContent("-");
            public static readonly GUIContent sharedTableData = EditorContent.TextContent("Shared Table");
        }
        ...
        public virtual void OnEnable()
        {
            ...
            m_TableEditorButton = EditorContent.TextContent("Open Table Editor", EditorGUIUtility.ObjectContent(target, target.GetType()).image);
            ValidateTableData();
        }
        ...
        void ResolveTableCollection()
        {
            ...
            {
                m_CollectionButton = EditorContent.TextContent("Select Collection", EditorGUIUtility.ObjectContent(m_Collection, m_Collection.GetType()).image);
                return;
            }
